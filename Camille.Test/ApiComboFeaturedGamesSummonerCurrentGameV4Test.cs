...
namespace Camille.Test
{
    ...
    public class ApiComboFeaturedGamesSummonerCurrentGameV4Test : ApiTest
    {
        ...
        public void Get()
        {
            var featured = Api.SpectatorV4.GetFeaturedGames(Region.NA);
            CheckFeatured(featured);
            foreach (var gameInfo in featured.GameList)
            {
                var participant = gameInfo.Participants[0];
                var summoner = Api.SummonerV4.GetBySummonerName(Region.NA, participant.SummonerName);
                var currentGame = Api.SpectatorV4.GetCurrentGameInfoBySummoner(Region.NA, summoner.Id);
        ...
        {
            var featured = await Api.SpectatorV4.GetFeaturedGamesAsync(Region.NA);
            CheckFeatured(featured);
            var tasks = featured.GameList.Select(async gameInfo =>
            {
                var participant = gameInfo.Participants[0];
                var summoner = await Api.SummonerV4.GetBySummonerNameAsync(Region.NA, participant.SummonerName);
                var currentGame = await Api.SpectatorV4.GetCurrentGameInfoBySummonerAsync(Region.NA, summoner.Id);
                Assert.IsNotNull(currentGame);
                Assert.AreEqual(gameInfo.GameId, currentGame.GameId);
                Assert.IsTrue(currentGame.Participants.Any(cp => participant.SummonerName.Equals(cp.SummonerName)),
                    "Failed to find matching summoner.");
